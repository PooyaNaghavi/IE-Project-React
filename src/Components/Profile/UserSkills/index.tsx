    ...
    render() {
        ...
        return (
            <div>
                ...
                {userId === '1' && (<div className="row">
                    <div className="col-1 dummy"></div>
                    <div className="col-10">
                        <div id="add-skill-container">
                            <div id="maharat-ha">مهارت‌ها:</div>
                            <form id="add-skill-form" onSubmit={e => this.handleAddSkillSubmit(e)} method="POST">
                                <select name="add-skill-select" value={this.state.skillToAdd} onChange={e => this.handleSelectChange(e)} id="add-skill-select">
                                    <option value="" disabled hidden>-- انتخاب مهارت‌ها --</option>
                                    {
                                        this.props.allSkills.map(skill => (
                                            this.userHasThisSkill(skill)
                                                ? (<option key={skill.name} style={{ display: 'none' }}></option>)
                    ...
                    <div className="col-10">
                        <div id="skills-container">
                            {skillsList.map(skill => {
                                const endorsed = !!(skill.endorseUsers.filter((u: any) => u.id === '1').length)
